...
def daily_setups(request):
    if request.method == 'GET':
        last_5_entries = DailyStep.objects.order_by('-date_created')[:5]
        serialized_setups = serializers.serialize('json', last_5_entries)
...
def trade_breakdown(request):
    if request.method == 'GET':
        last_5_entries = TradeBreakdown.objects.order_by('-date_created')[:5]
        serialized_setups = serializers.serialize('json', last_5_entries)
